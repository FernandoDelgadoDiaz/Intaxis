...
export async function loadBusinessSnapshot(supabase, businessId) {
  ...
  const [products, ingredients, recipes, recipeItems, movements, capacity, decisions, hypotheses] = await Promise.all(queries);
  for (const result of [products, ingredients, recipes, recipeItems, movements, capacity, decisions, hypotheses]) {
    if (result.error) throw result.error;
  }
  const stockByIngredient = new Map();
  for (const movement of movements.data || []) {
    stockByIngredient.set(movement.ingredient_id, Number(stockByIngredient.get(movement.ingredient_id) || 0) + Number(movement.quantity_delta || 0));
  }
  return {
